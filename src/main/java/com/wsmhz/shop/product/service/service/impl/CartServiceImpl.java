...
import com.wsmhz.common.business.utils.BigDecimalUtil;
...
import com.wsmhz.shop.product.service.mapper.CartMapper;
...
public class CartServiceImpl extends BaseServiceImpl<Cart> implements CartService {
    ...
    public CartVo selectUserCart(Long userId, Boolean checked) {
        List<CartSimpleVo> cartList = selectByUserId(userId,checked);
    ...
    public List<CartSimpleVo> selectByUserId(Long userId, Boolean checked) {
        Example example = new Example(Cart.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("userId",userId);
        if(checked != null){
        ...
        return DozerBeanUtil.map(cartList, CartSimpleVo.class);
    }
